...
describe("IssueWorkflow (mock workers, no git)", () => {
  ...
  it("/repro dispatch honors a worker override", async () => {
    const provider = new FakeProvider(issueWithSteps(REPRO_OVERRIDE_ISSUE));
    const wf = new IssueWorkflow({ provider, config: { defaultWorker: "claude" } });

    await wf.dispatch(
      { provider: "github", id: String(REPRO_OVERRIDE_ISSUE) },
      parseIssueCommand("/repro codex"),
    );

    const report = readFileSync(runPaths(REPRO_OVERRIDE_ISSUE).report, "utf8");
    expect(report).toContain("Worker used: codex");
